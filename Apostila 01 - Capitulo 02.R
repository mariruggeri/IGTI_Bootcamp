...
# A função a seguir gera quantidades aleatórias de sucesso oriundos de uma quantidade (size) 
# de tentativas dada a probabilidade (prob) de sucesso.
...
hist(va_binomial) 
# Ajuste o parametro n para 1000 e plote o histograma, 
# observe como a distribuição binomial se aproxima da normal
# Podemos também querer a probabilidade de que até dois clientes comprem.
va_binomial_n1000 <- rbinom(n = 1000, size=10, prob=0.5)
hist(va_binomial_n1000)
...
#########################################
# Exemplo: Uma loja recebe em média, 6 (𝝺) clientes por minuto.
...
# sigam uma distribuição normal com média 𝜇=2.500 e desvio padrão σ= 170.
# Ao selecionar aleatoriamente um indivíduo dessa população, qual a probabilidade
# de ter salário entre #2.400 e 2.600 ?
# Precisamos achar a probabilidade do indivíduo ter um salário de até 2.600 
# e subtrair pela probabilidade do indivíduo ter o salário até 2.400
...
#P(X<=2600) - P(X<=2400) probabilidade_ate_2600 - probabilidade_ate_2400
...
# Ao aplicar o comando na variável va_normal que acabmos de criar, 
# ela ficará com média zero e desvio padrão unitário
va_normal_padrao <- scale(va_normal)
hist(va_normal_padrao)

# Exemplo: Suponha que a distribuição dos salários dos funcionários 
# de uma empresa sigam uma distribuição
...
# salário acima de 2.600 ? #Padronização
z <- (2600-2500)/170
...
# Gera uma linha sobre média, que é zero pois padronizamos a distribuição
abline(v = 0,col = 'blue') 
# Gera uma linha sobre o valor z obtido
...
# df2 é o segundo grau de liberdade hist(va_f)
