...
# Obter o intervalo de confiança para uma variável cuja média = 30, 
# desvio padrão = 7,31 e n = 30
...
#Vamos armazenar os valores em objetos media <- 30
...
# Utilizando o exemplo da apostila, onde calculamos o intervalo para proporção onde # 138 de n = 500 clientes 
# realizaram a devolução do produto
...
# Essa variável é um vetor para armazenar a média de cada subamostra bootstrap
medias <- c() 

# E é o númer de subamostras extraídas para gerar a distribuição amostral de médias
R <- 1000 
...
# do menor valor para o maior e achar o valor que deixará 2,5% dos dados para trás eo
...
# Inicializa variavel para armazenar as médias de cada subamostra medias <- c()
...
IC<-t.test(va, conf.level = 0.95) IC$conf.int